...
<table>
    ...
    <tbody<
        <tr>
            ...
            <td>カスタマイズ情報を初期化</td>
            <td>
                指定したユーザーのカスタマイズ情報をすべて強制的に消去します。(参照: 「9 カスタマイズ情報の初期化」)
            </td>
        </tr>
        <tr>
...
初期画面をロール毎に切り替える必要がなく、全てのユーザーの初期画面を同じに設定する場合は、「defaultRole」の編集画面を開き、「5 コマンドバーの構成」以降を読み進めてください。
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                ロールの種類を選択します。<br>
                主体者は設定によりHTTPヘッダの値にマッピングされます。設定方法は<a href="../installation-guide/customizing-web-application-module.md" title="Webアプリケーションモジュールのカスタマイズ">Webアプリケーションモジュールのカスタマイズ</a>「5 HTTPヘッダをロール情報として定義する」を参照ください。
            </td>
        </tr>
        <tr>
            <td>正規表現</td>
...
* [__編集__]: ティッカーとランキング、追加したリンクとHTMLにのみ編集アイコンが表示されます。別途設定が必要です。
    ...
    * リンクとHTMLについては「5.2 リンク追加/HTML追加」を参照してください。
...
「5 コマンドバーの構成」の手順によってタブの構成を変更した後、ユーザーがブラウザを再ロードした際、ユーザー画面のコマンドバーが上書き更新されます。


#### 8.2.2 タブの追加

「6.1 タブの追加」の手順で固定タブを追加した場合、ユーザーの画面には、ユーザーがブラウザを再ロードした際に、追加した固定タブは固定タブと動的タブ(ユーザーが追加したタブ)の間に追加されます。
...
「6.2 タブの削除」の手順で固定タブを削除した場合、ユーザーの画面では固定タブは削除されず、削除された固定タブは固定エリアのなくなった動的タブ(ユーザーが追加したタブ)になります。また、動的タブに変更されたタブはもっとも右側に自動的に移動します。
...
「7 タブの構成」の手順によってタブの構成を変更した後、ユーザーがブラウザを再ロードした際、以下の規則でユーザー画面に反映されます。
